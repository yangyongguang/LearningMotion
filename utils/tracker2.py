...
class Node:
    """
        ...
        2) box (a box (x, y, h, w, theta))
        3) label (active label indicating keeping the feature)
        4) detection, the formatted box
    """
...
class FeatureRecorder:
    """
        Record features and bboxes every frame
    """

    def __init__(self):
        self.max_record_frame = configs.tracker.Max_record_frame
    ...
    def update(self, model, frame_index, features, boxes):
        ...
        if frame_index not in self.all_frame_idx:
            ...
            # add new item for all_frame_index, all_features and all_boxes. Besides,
            # also add new similarity
            ...
            for pre_index in self.all_frame_index[:-1]: #  iter all pre index expect curr input frame_index
                m_frame = 5
                if frame_index - pre_index < m_frame:
                    delta = pow(configs.tracker.decay, (frame_index - pre_index) / 3.0)
                pre_similarity = model.forward_stacker_features(self.all_features[pre_index],
...
class STrack(BaseTrack):
    ...
    def __init__(self,
                 ...
                 ddd_submission=None):
        #  wait activate
        self._xylwr = np.array(xylwr, dtype=np.float)
        ...
        self.observations = []
        self.observations_xylwr = []
        self.observations_xylwr.append(self._xylwr.copy())
    ...
    def get_similarity(self, frame_index, recorder):  # need understand
        similarity = []
        for n in self.nodes:
            f = n.frame_index
            id = n.id

            if frame_index - f >= configs.tracker.max_track_node:
                continue
    ...
    def prediction_at_frame_box(self, frame_id):  # prediction_at_frame_tlbr
    ...
    def activate(self, kalman_filter, frame_id):
        ...
        if self.use_lstm:
            self.kalman_filter = KalmanFilterLSTM()
...
class Tracker(object):
    def __init__(self, model, frame_rate=10):
        ...
        self.frame_id = 0
        self.buffer_size = int(frame_rate / 30.0 * configs.tracker.track_buffer)
        self.max_time_lost = self.buffer_size
        self.max_per_image = configs.tracker.max_object
        ...
        if self.use_lstm:
            STrack.shared_kalman_lstm = KalmanFilterLSTM()
            self.kalman_filter = KalmanFilterLSTM()
            ...
            if len(dets) > 0:
                ...
                detections = [
                    ...
                    for (
                        ...
                        dets[:, :5], nodes, ddd_boxes, ddd_org_boxes, submission
                    )
                ]
                ...
                features = self.model.forward_feature_extracter(FeatureMaps, detection_centers)
